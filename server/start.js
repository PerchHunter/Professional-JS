const exp = require('constants');
const express = require('express');
const fs = require('fs');

const port = 3000;
const static_dir = '../public';
const app = express();

app.use(express.json());
app.use(express.static(static_dir));

app.get('/catalogData', (req, res) => {
    fs.readFile('data/catalog.json', 'utf-8', (err, data) => {
        res.send(data);
    })
})

app.get('/getBasket', (req, res) => {
    fs.readFile('data/cart.json', 'utf-8', (err, data) => {
        res.send(data);
    })
})

app.post('/addToCart', (req, res) => {
    fs.readFile('data/cart.json', 'utf-8', (err, data) => {
        const cart = JSON.parse(data);
        let id = 1;

        if (cart.length > 0) {
            id += cart.length;
        }

        const item = req.body;
        console.log(item)
        item.id_product = id;
        cart.push(item);

        fs.writeFile('data/cart.json', JSON.stringify(cart), err => {
            console.log(`${item.product_name} добавлен в корзину`);
            res.end();
        })
    })
})

app.post('/deleteFromBasket', (req, res) => {
    fs.readFile('data/cart.json', 'utf-8', (err, data) => {
        const cart = JSON.parse(data);
        const index = cart.findIndex((good) => good.id_product === req.body.id_product);

        cart.splice(index, 1);

        fs.writeFile('data/cart.json', JSON.stringify(cart), err => {
            console.log(`Удалено.`);
            res.end();
        })
    })
})

// app.post('/stats', (req, res) => {
//     fs.readFile('data/stats.json', 'utf-8', (err, data) => {
//         const stats = JSON.parse(data);

//         req.body.numberInOrder = stats.length + 1;
//         stats.push(req.body);

//         fs.writeFile('data/stats.json', JSON.stringify(stats), err => {
//             res.end();
//         })
//     })
// })

app.listen(port, () => console.log(`Запустили сервер на ${port} порту.`));